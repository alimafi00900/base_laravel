<div class="form-group col-md-6 col-lg-4 col-12">
    ...
    @endif
    <textarea type="text"  @if(getCurrentStructure(['fields',$field_key,"requests","add_submit","status"])===true and
    ...
    {!! $property_kay !!}="{{$property_value}}"
              @endforeach
              @endif
    class="form-control"  name="{{$field_key}}">@if(isValidValue(getProperty($field_value,"default_value"))){{getProperty($field_value,"default_value")}}@endif</textarea>
</div>
